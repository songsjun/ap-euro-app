...
export function UnitReviewClient({ unit, unitTitle }: { unit: number; unitTitle: string }) {
  const [ready, setReady] = useState(false)
  useEffect(() => { ensureAppReady().then(() => setReady(true)).catch(console.error) }, [])
    ...
    <div className="min-h-screen">
      <div className="sticky top-0 z-10 bg-white/90 dark:bg-stone-900/90 backdrop-blur border-b border-stone-100 dark:border-stone-800">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          <Link href={`/unit/${unit}`} className="text-stone-400 hover:text-stone-600 dark:hover:text-stone-300 transition-colors">
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
        <TopicProvider unit={unit} topicId="" isUnitReview={true}>
          <TopicListView unit={unit} topicId="" isUnitReview={true} />
        </TopicProvider>
      </div>
    </div>
  )
