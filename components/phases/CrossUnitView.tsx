  ...
  useEffect(() => {
    ensureAppReady().then(async () => {
      ...
      const [res, allCompletions] = await Promise.all([
        repo.getResourcesByTiming('cross_unit'),
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-6">
        {!ready ? (
          ...
          <>
            <ResourceList resources={aResources} completions={completions} saving={saving} onToggle={toggleDone} />
            {bResources.length > 0 && (
              <div>
...
function ResourceList({ resources, completions, saving, onToggle }: {
  resources: Resource[]
          ...
          <div key={r.id}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <p className={`text-xs ${meta?.color ?? 'text-stone-400'}`}>{meta?.label ?? r.source}</p>
                <span className="text-stone-300 dark:text-stone-600">·</span>
                <p className="text-xs text-stone-400 dark:text-stone-500">{typeLabel}</p>
              </div>
            </div>
            {r.paid && (
